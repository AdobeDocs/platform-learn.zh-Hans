...
第17行将Offer Decisioning返回的映像应用到该网站。 单击&#x200B;**[!UICONTROL 保存]**。
...
在执行此更改后，Adobe Experience Platform数据收集中的此规则现在将侦听Offer Decisioning的响应，该响应是Web SDK响应的一部分，收到响应后，将在主页上显示选件的图像。
...
- 1 Offer Decisioning实施（使用您的数据收集属性）

在下一个练习中，您将了解如何将在Adobe Journey Optimizer中创建的优惠和决策与Adobe Target体验定位活动相结合。

## 后续步骤

转到[3.3.4将Adobe Target和Offer Decisioning结合使用](./ex4.md){target="_blank"}
