...
在下一组查询中，我们将扩展上述查询，以便完整查看访问“取消服务”页面的客户及其行为。 您将了解如何使用Adobe定义的函数来会话化信息，识别事件的顺序和时间。 此外，您还将连接多个数据集，以进一步扩充和准备数据以供在Microsoft Power BI中进行分析。
...
接下来，您将保留查询（也称为&#x200B;**创建表作为select**&#x200B;或&#x200B;**CTAS**），作为您将在Microsoft Power BI中使用的新数据集。

## 后续步骤

转到[2.1.5从查询生成数据集](./ex5.md){target="_blank"}
