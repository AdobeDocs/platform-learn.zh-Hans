...
完成连接后，您现在可以继续影响可视化图表。 Adobe Analytics和CJA的区别在于，CJA需要数据视图，以便在可视化之前清理和准备数据。
...
| 量度 | commerce.purchases.value | 购买 | `commerce.purchases.value` |
...
让我们用CJA中的Analysis Workspace回答这些问题以及其他一些问题。
...
>[!NOTE]
>
>如果您刚刚创建了&#x200B;**数据连接**&#x200B;和&#x200B;**数据视图**，则可能需要等待几个小时。 当有大量的数据记录时，CJA需要一些时间来回填历史数据。
...
接下来，让我们分析使用&#x200B;**忠诚度级别**&#x200B;为&#x200B;**铜级**&#x200B;的&#x200B;**有机搜索**&#x200B;访问网站的客户使用的是哪个&#x200B;**设备类型**。 将Dimension **设备类型**&#x200B;拖放到&#x200B;**铜牌**&#x200B;上。 您随后将看到以下内容：
...
## 1.2.5.3.2 Funnel或流失分析

漏斗是一种非常好的机制，可用于了解客户历程中的主要步骤。 这些步骤也可以来自离线交互（例如，呼叫中心），然后您可以将其与同一funnel中的数字接触点组合。
...
让我们看一看您可以做的另一件事：本月我的客户历程Funnel与上个月的比较如何？ 移动设备和桌面设备呢？
...
您将看到我们正在按设备类型比较不同时间段（一月和二月）的funnel。

此类分析无法在Google Analytics UI中执行，或者非常有限。 因此，CJA再次为Google Analytics捕获的数据增加了大量价值。
...
作为分析师，假设您想要了解主要电子商务funnel的最新动态：主页>内部搜索>产品详细信息>结账>购买。

我们来看看如何在funnel中添加一些新步骤。 为此，请打开&#x200B;**页面名称**&#x200B;维度。
...
现在，我们来设置一个多路径营销渠道流量分析。 将&#x200B;**营销渠道**&#x200B;维度拖放到&#x200B;**登入维度**&#x200B;区域。
