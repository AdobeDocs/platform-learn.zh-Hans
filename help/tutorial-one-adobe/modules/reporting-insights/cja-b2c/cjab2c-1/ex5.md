...
在本练习中，您将使用CJA中的Analysis Workspace来分析产品查看、产品漏斗、流失情况等。

我们将介绍在“模块7 — 查询服务”中完成的一些查询，这样您就可以了解运行相同的查询以及更多查询有多么容易，但是请不要使用SQL并且只依赖于Analysis Workspace的拖放原则。

让我们使用您在Analysis Workspace[中的](./ex4.md)1.1.4数据准备中创建的项目，转到[https://analytics.adobe.com](https://analytics.adobe.com)。
...
在这里你可以做很多事。 一些示例：随时间比较、按设备比较每个步骤或按忠诚度比较。 但是，如果您想要分析一些有趣的内容，例如为什么客户在将项目添加到购物车后不购买，我们可以使用CJA中的最佳工具：右键单击。
...
由于第一个流量分析，出现了一个新问题。 因此，我们决定创建此表格并根据区段检查一些KPI来回答该问题。 如您所见，使用insight的时间比使用SQL或其他BI解决方案快得多。
