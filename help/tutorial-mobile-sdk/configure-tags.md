...
Adobe Experience Platform中的标记是Adobe推出的新一代标记管理功能。 标记为客户提供了一种简单的方式来部署和管理用来加强相关客户体验的分析、营销和广告标记。 在产品文档中了解有关[标记](https://experienceleague.adobe.com/docs/experience-platform/tags/home.html?lang=zh-Hans)的更多信息。
...
1. 在数据收集界面中创建新的移动标记属性：
   ...
   1. 选择&#x200B;**[!UICONTROL 保存]**。
      ...
      >[!NOTE]
      >
      > 基于Edge的Mobile SDK实施（如您在本课程中正在执行的实施）的默认同意设置来自[!UICONTROL 同意扩展]，而不是标记属性配置中的[!UICONTROL 隐私]设置。 您可在本课程的后面部分添加并配置同意扩展。 有关详细信息，请参阅[文档](https://developer.adobe.com/client-sdks/edge/consent-for-edge-network/)。
...
1. 检查扩展：
   ...
   1. 选择&#x200B;**[!UICONTROL 已安装]**&#x200B;选项卡。
      ...
      已安装![个标记](assets/tags-installed.png)
...
1. 使用![搜索](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Search_18_N.svg) **[!UICONTROL 搜索]**&#x200B;字段查找并安装&#x200B;**AEP保证**&#x200B;扩展。
...
1. 使用![搜索](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Search_18_N.svg) **[!UICONTROL 搜索]**&#x200B;字段查找并安装&#x200B;**Adobe Experience PlatformEdge Network**&#x200B;扩展。

   1. 在&#x200B;**[!UICONTROL 数据流]**&#x200B;中，选择您在[上一步](create-datastream.md)中为每个环境创建的&#x200B;**[!UICONTROL 数据流]**，例如&#x200B;**[!DNL Luma Mobile App]**。
...
您的库是为新扩展和配置构建的。 <span style="color:green">●</span>在&#x200B;**[!UICONTROL 初始生成]**&#x200B;按钮中指示生成成功。
...
1. 您可以复制![复制](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Copy_18_N.svg)使用CocoaPods设置项目的说明。 CocoaPod用于管理SDK版本和下载。 若要了解更多信息，请查阅[CocoaPods文档](https://cocoapods.org/)。 如果您使用Android™作为开发平台，则Gradle是用于管理SDK版本、下载和依赖项的工具。 要了解更多信息，请查阅[Gradle文档](https://gradle.org/)

   安装说明为您提供了一个良好的实施起点。 您可以在[此处](https://developer.adobe.com/client-sdks/documentation/getting-started/get-the-sdk/)查找其他信息。

   >[!INFO]
   >
   >在本教程的其余部分中，您&#x200B;**不会**&#x200B;使用CocoaPods说明，而是使用基于本机Swift Package Manager (SPM)的设置。
   >

1. 选择&#x200B;**[!UICONTROL Add Initialization Code]**&#x200B;下的&#x200B;**[!UICONTROL Swift]**&#x200B;选项卡。 此代码块显示了如何导入所需的SDK并在启动时注册扩展。 [安装SDK](install-sdks.md)中对此进行了详细介绍。
...
>[!NOTE]
>
>应将安装说明视为起点，而不是最终的文档。 可以在官方的[文档](https://developer.adobe.com/client-sdks/home/)中找到最新的SDK版本和代码示例。
...
* 在移动标记属性中，规则和配置将渲染到云中托管的JSON文件中。 JSON文件由移动设备应用程序中的Mobile Core扩展下载和读取。 扩展是相互协作的分隔SDK。 如果向标记属性添加扩展，则还必须更新应用程序。 如果更改扩展设置或创建规则，则在发布更新的标记库后，这些更改将反映在应用程序中。 这种灵活性允许您修改设置(如Adobe Analytics报表包ID)，甚至可以更改应用程序的行为（使用数据元素和规则，如您将在后续课程中看到的），而无需更改应用程序中的代码并重新提交应用商店。
