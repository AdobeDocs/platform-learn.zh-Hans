...
Edge Network扩展提供了一个将Experience事件发送到Platform Edge Network的API。 体验事件是一个对象，其中包含符合XDM ExperienceEvent模式定义的数据。 更简单地说，它们捕获用户在您的移动应用程序中的操作。 Platform Edge Network收到数据后，即可将其转发到数据流中配置的应用程序和服务，如Adobe Analytics和Experience Platform。 进一步了解 [体验事件](https://developer.adobe.com/client-sdks/documentation/getting-started/track-events/) ，位于产品文档中。

## 先决条件

* 更新了包含所需SDK的PodFile。
* AppDelegate中注册的扩展。
* 已将MobileCore配置为使用您的开发AppId。
* 导入的SDK。
* 通过上述更改成功构建和运行应用程序。
...
查看以下示例，而不尝试在示例应用程序中实施它：

1. 在您的架构中，标识您尝试收集的事件，在本例中，我们跟踪的是产品视图。
...
1. 开始构建对象：
   ...
   * eventType：描述发生的事件，使用 [已知值](https://github.com/adobe/xdm/blob/master/docs/reference/classes/experienceevent.schema.md#xdmeventtype-known-values) 可能的话。
   * commerce.productViews.value：提供事件的数字值。 如果它是布尔值(或Adobe Analytics中的“计数器”)，则该值将始终为1。 如果是数值或货币事件，该值可以大于1。

1. 在您的架构中，标识与事件关联的任何其他数据。 在此示例中，包括 `productListItems` 这是一组用于商业相关事件的标准字段：
   ![产品列表项架构](assets/mobile-datacollection-prodListItems-schema.png)
   * 请注意 `productListItems` 是一个阵列，可以提供多个产品。
...
1. 在您的架构中，标识您尝试收集的事件。 在此示例中，跟踪由应用程序操作事件和名称组成的“应用程序交互”。
...
上述实例有望解释在构建XDM数据对象时的思想过程。 接下来，我们将在Luma应用程序中添加屏幕视图跟踪。
...
1. 执行该操作并查找 `hitReceived` 事件来自 `com.adobe.edge.konductor` 供应商。
1. 选择事件并查看 `messages` 对象。
...
>[!NOTE]
>
>为清楚起见，所有值都进行了硬编码。 在真实世界中，这些值会动态填充。
...
现在，您已收集事件并将它们发送到Platform Edge Network，它们将被发送到中配置的应用程序和服务。 [数据流](create-datastream.md). 在后面的课程中，您会将此数据映射到 [Adobe Analytics](analytics.md) 和 [Adobe Experience Platform](platform.md).
