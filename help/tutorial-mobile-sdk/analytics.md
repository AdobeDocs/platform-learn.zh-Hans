---
title: 将通过Platform Mobile SDK收集的数据映射到Adobe Analytics
...
您在前面的课程中收集并发送到Platform Edge Network的[event](events.md)数据将转发到您在数据流中配置的服务，包括Adobe Analytics。 您可以将数据映射到报表包中的正确变量。

![架构](assets/architecture-aa.png){zoomable="yes"}
...
要将XDM数据从Edge Network发送到Adobe Analytics，请将Adobe Analytics服务配置到您在[创建数据流](create-datastream.md)过程中设置的数据流。
...
1. 选择&#x200B;**[!UICONTROL 保存]**。
   ...
   ![将Adobe Analytics添加为数据流服务](assets/datastream-service-aa.png){zoomable="yes"}
...
如果仔细查看，所有事件都有两个字段：`value`（必需）和`id`（可选）。 `value`字段用于递增事件计数。 `id`字段用于序列化。
...
1. 查看ExperienceEvent点击。
   ...
   ![分析xdm点击](assets/analytics-assurance-experiencevent.png){zoomable="yes"}
...
1. 查看&#x200B;**[!UICONTROL analytics.mapping]**&#x200B;事件。
   ...
   ![分析xdm点击](assets/analytics-assurance-mapping.png){zoomable="yes"}

在Analytics映射中注意以下事项：

* 已基于&#x200B;**[!UICONTROL 使用]**&#x200B;填充`scAdd`事件`commerce.productListAdds`。
...
转发到Analytics的XDM数据将转换为[上下文数据](https://github.com/Adobe-Marketing-Cloud/mobile-services/blob/master/docs/ios/getting-started/proc-rules.md?lang=en)，包括标准字段和自定义字段。
...
* 将&#x200B;**[!UICONTROL Adobe Analytics ExperienceEvent完整扩展]**&#x200B;字段组添加到您的架构中。
  ...
  ![Analytics ExperienceEvent FullExtension字段组](assets/schema-analytics-extension.png){zoomable="yes"}
...
>[!TIP]
>
>与以前的移动应用程序实施不同，页面/屏幕查看次数与其他事件之间没有区别。 相反，可以通过在处理规则中设置&#x200B;**[!UICONTROL 页面名称]**&#x200B;维度来递增&#x200B;**[!UICONTROL 页面查看]**&#x200B;量度。 由于您正在教程中收集自定义`screenName`字段，因此强烈建议在处理规则中将屏幕名称映射到&#x200B;**[!UICONTROL 页面名称]**。
...
>[!SUCCESS]
>
>您已设置应用程序，通过在数据流中启用Edge服务将Experience Adobe Analytics XDM对象映射到Adobe Analytics变量。 并在适用的情况下使用处理规则。<br/>感谢您投入时间学习Adobe Experience Platform Mobile SDK。 如果您有任何疑问、希望分享一般反馈或有关于未来内容的建议，请在此[Experience League社区讨论帖子](https://experienceleaguecommunities.adobe.com:443/t5/adobe-experience-platform-data/tutorial-discussion-implement-adobe-experience-cloud-in-mobile/td-p/443796)上分享这些内容。
