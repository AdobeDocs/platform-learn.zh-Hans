...
| `mbox3rdPartyId` | 在`xdm.identityMap`对象中设置 | 用于跨设备和客户属性同步Target配置文件。 必须在数据流[的](https://experienceleague.adobe.com/docs/experience-platform/edge/personalization/adobe-target/using-mbox-3rdpartyid.html)Target配置中指定用于客户ID的命名空间。 |
...
实体参数用于为Target Recommendations传递行为数据和补充目录信息。 Platform Web SDK也支持at.js支持的所有[实体参数](https://experienceleague.adobe.com/docs/target/using/recommendations/entities/entity-attributes.html)。 与配置文件参数类似，所有实体参数都应在Platform Web SDK `data.__adobe.target`命令有效负载中的`sendEvent`对象下传递。
