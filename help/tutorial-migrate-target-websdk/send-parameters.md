...
这些页面的Target参数使用Platform Web SDK以不同方式发送。 可以使用at.js通过多种方法将参数传递到Target：
...
Platform Web SDK提供了一种统一的数据发送方式，无需额外函数。 必须使用`sendEvent`命令在有效负载中传递所有参数，并且这些参数属于两个类别：
...
| `mbox3rdPartyId` | 在`xdm.identityMap`对象中设置 | 用于跨设备和客户属性同步Target配置文件。 必须在数据流[的](https://experienceleague.adobe.com/docs/experience-platform/edge/personalization/adobe-target/using-mbox-3rdpartyid.html?lang=zh-Hans)Target配置中指定用于客户ID的命名空间。 |
...
>[!NOTE]
>
>由于自定义mbox参数是`xdm`对象的一部分，因此您需要更新任何受众、活动或使用新名称引用这些mbox参数的配置文件脚本。 有关详细信息，请参阅本教程的[更新Target受众和配置文件脚本以获得Platform Web SDK兼容性](update-audiences.md)页。


## 轮廓参数

必须在Platform Web SDK `data.__adobe.target`命令有效负载中的`sendEvent`对象下传递Target配置文件参数。
...
实体参数用于为Target Recommendations传递行为数据和补充目录信息。 Platform Web SDK也支持at.js支持的所有[实体参数](https://experienceleague.adobe.com/docs/target/using/recommendations/entities/entity-attributes.html?lang=zh-Hans)。 与配置文件参数类似，所有实体参数都应在Platform Web SDK `data.__adobe.target`命令有效负载中的`sendEvent`对象下传递。
...
>[!IMPORTANT]
>
> 必须使用`_experience.decisioning.propositionEventType`设置`display: 1`，才能使用调用递增Target量度。
...
Target允许使用单个客户ID跨设备和系统同步配置文件。 使用at.js时，可以将其设置为Target请求中的`mbox3rdPartyId`，或设置为发送到Experience Cloud Identity服务的第一个客户ID。 与at.js不同，Platform Web SDK实施允许您指定在存在多个客户ID时用作`mbox3rdPartyId`的客户ID。 例如，如果贵企业有一个全局客户ID，并且不同业务线的客户ID各不相同，则可以配置Target应使用哪个ID。
...
1. 使用`sendEvent`字段组执行`identityMap`命令
