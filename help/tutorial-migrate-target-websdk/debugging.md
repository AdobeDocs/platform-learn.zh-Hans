...
验证Target活动并调试Web SDK，以解决实施、内容交付或受众资格问题。 此迁移指南页面介绍了使用at.js调试与使用Platform Web SDK调试之间的差异。
...
| `mboxTrace` URL参数 | 是 | 使用Experience Platform Debugger浏览器扩展 |
...
Adobe Experience Platform Debugger可以安装在Google Chrome中。 请按照下面的相应链接安装扩展：

- [Chrome](https://chrome.google.com/webstore/detail/adobe-experience-platform/bfnnokhpnncpkdmbokanobigaccjkpob)

安装Chrome扩展或Firefox加载项后，会在扩展栏中添加一个图标(![](assets/start-icon.jpg))。 选择此图标以打开扩展。

请参阅专用指南，了解有关[Adobe Experience Platform Debugger扩展](https://experienceleague.adobe.com/docs/experience-platform/debugger/home.html?lang=zh-Hans)以及如何调试所有Adobe Web应用程序的更多信息。
...
>[!CAUTION]
>
>平台Web SDK版本2.13.0或更高版本支持Target QA模式功能。 已根据`xdm.web.webPageDetails.URL`调用中传递的`sendEvent`值启用目标QA模式。 对此值所做的任何修改（例如将所有字符变为小写），都可能会妨碍Target QA模式正常工作。
...
>[!NOTE]
>
>上面列出的所有at.js调试功能都可以通过Adobe Experience Platform Debugger中的增强功能使用。

### 禁用Target功能

Platform Web SDK当前没有选择性地禁止Target响应的功能。 但是，可以使用浏览器的开发人员工具、各种浏览器扩展或第三方应用程序阻止Platform Web SDK请求。 例如，要使用Google Chrome阻止平台Web SDK，请执行以下操作：

1. 右键单击页面上的任意位置，然后选择&#x200B;**检查**
1. 选择&#x200B;**网络**&#x200B;选项卡
...
>[!NOTE]
>
>at.js和Platform Web SDK的调试日志提供了类似级别的详细信息，但值得注意的异常是，Web SDK仅通知由于选择器无效而出现的渲染错误。 调试日志记录当前未确认渲染成功。

### 查看Target跟踪

Target跟踪可提供有关活动资格和访客的Target配置文件的详细信息。 由于Target跟踪包含不公开的信息，因此查看这些跟踪需要授权令牌或在Adobe Experience Platform Debugger浏览器扩展窗口中进行身份验证。

| 目标跟踪方法 | at.js | Platform Web SDK |
| --- | --- | --- |
| `mboxTrace` URL参数 | 是 | 否 |
...
可在Adobe Experience Platform Debugger浏览器扩展和Assurance应用程序（以前称为项目Griffon）中查看Target跟踪信息。 要在Assurance中查看Target跟踪，请执行以下操作：

1. 如上所述，打开Adobe Experience Platform Debugger浏览器扩展并连接远程调试会话
1. 选择带有您的会话名称的链接，该链接位于调试日志上方
1. Platform Assurance会加载并在数据流中为您的实施配置的所有Adobe应用程序并显示详细的日志记录
1. 按`adobe.target`筛选日志
1. 选择类型为`com.adobe.target.trace`的日志条目
1. 展开有效负载的详细信息并查看`context > targetTrace`下的信息

![如何使用Assurance查看Target跟踪](assets/target-trace-assurance.png){zoomable="yes"}
...
- 决策范围和活动详细信息位于建议数组中
- 选件详细信息位于`items`下的`data`数组中
- 响应令牌位于`items`下的`meta`数组中
