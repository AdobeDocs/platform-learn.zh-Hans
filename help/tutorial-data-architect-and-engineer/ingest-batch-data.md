...
1. 从本地`luma-crm.json`文件夹中选择下载的`Luma Tutorial Assets`
...
最后，通过按照`Luma CRM Id`命名空间查找其中一个配置文件来确认您的其中一个配置文件已创建，例如`f660ab912ec121d1b1e928a0bb4bc61b`
...
再次在`5625458`命名空间中查找值为`loyaltyId`的配置文件，查看其配置文件中是否有任何购买事件。 您应该会看到一次购买。 您可以通过选择&#x200B;**[!UICONTROL 查看JSON]**&#x200B;来深入了解购买的详细信息：
