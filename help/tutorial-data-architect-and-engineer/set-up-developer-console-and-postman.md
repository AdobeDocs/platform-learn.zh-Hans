...
要完成本教程中的API练习，[下载适用于您的操作系统的Postman应用程序。](https://www.postman.com/downloads/)使用Experience Platform API不需要使用Postman，但API工作流更简单，Adobe Experience Platform提供了几十个Postman收藏集来帮助您执行API调用并了解其操作方式。 本教程的其余部分假定您了解Postman的一些工作知识。 如需帮助，请参阅[Postman文档](https://learning.postman.com/)。
...
Adobe Developer Console是开发人员目标，可用于访问Adobe API和SDK、侦听近乎实时的事件、在运行时运行函数或构建插件或App Builder应用程序。 您将使用它来访问Experience Platform API。 有关详细信息，请参阅[Adobe Developer Console文档](https://www.adobe.io/apis/experienceplatform/console/docs.html)
...
1. 在可用API列表中，选择&#x200B;**[!UICONTROL Experience Platform API]**，然后选择&#x200B;**[!UICONTROL 下一步]**。
...
1. API凭据([!UICONTROL )的]Name`Credential in Luma Tutorial API Project`
...
1. 如果列表较长，请使用教程参与者提供的`Credential in Luma Tutorial API Project`技术帐户电子邮件[!UICONTROL 进行筛选，以查找]凭据
1. 选择凭据
1. 选择&#x200B;**[!UICONTROL 保存]**
...
1. 切换到Web浏览器并查找公司的租户ID，方法是转到Experience Platform的界面并提取@符号&#x200B;*后面的URL*&#x200B;部分。 例如，我的租户ID为`techmarketingdemos`，但您的不同：
...
Adobe提供了一组丰富的[!DNL Postman]收藏集，可帮助您探索Experience Platform的API。 这些收藏集位于[Adobe Experience Platform Postman Samples GitHub存储库](https://github.com/adobe/experience-platform-postman-samples)中。 您应该将此存储库加入书签，因为在本教程中，您将多次使用此存储库，之后当您为自己的公司实施Experience Platform时，也会使用此存储库。

第一个收藏集可与Adobe Identity Management Service (IMS) API配合使用。 这是一种从Postman中检索访问令牌的便捷方法。
...
1. 访问令牌应自动存储为&#x200B;**环境的** ACCESS_TOKEN[!DNL Postman]环境变量。
...
在GitHub[中打开 [!DNL Postman] Experience Platform](https://github.com/adobe/experience-platform-postman-samples/tree/master/apis/experience-platform)收藏集。 此页面上有许多适用于各种Platform API的收藏集。 我强烈建议将它加入书签。
