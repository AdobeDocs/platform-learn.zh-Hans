...
在本课程中，我们将创建身份命名空间并将身份字段添加到架构中。 完成此操作后，我们还将能够完成前一课程中的架构关系。
...
>[!NOTE]
>
>只有在构建实时客户配置文件时，才需要提供标识字段。 如果您只将数据摄取到数据湖中，则不需要使用这些参数。
...
让我们首先为Luma忠诚度模式创建一个命名空间：

1. 在平台用户界面中，转到 **[!UICONTROL 身份]** 在左侧导航中
1. 您会注意到有多个现成的可用身份命名空间。 选择 **[!UICONTROL 创建身份命名空间]** 按钮
1. 提供详细信息，如下所示
...
现在为Luma产品目录架构设置另一个命名空间，并提供以下详细信息：
...
>[!NOTE]
>
>通过Web SDK收集的数据对于在架构中标记身份字段的典型实践是一个例外。 Web SDK使用身份映射来标记身份 *实施方面* 这样我们就可以确定 `Luma Web Events Schema` 在Luma网站上实施Web SDK时。 在后面的课程中，我们将将Experience Cloud访客ID (ECID)收集为主要ID，将crmId收集为次要ID。

通过我们选择的主要身份，可以清楚地了解 `Luma CRM Schema` 可以连接到 `Luma Offline Purchase Events Schema` 因为他们都使用 `loyaltyId` 作为标识符。 但我们如何才能将线下购买与在线行为联系起来？ 我们如何对随产品目录一起购买的产品进行分类？ 我们将使用其他标识字段和架构关系。

<!--use a visual-->

### 为辅助标识标记XDM字段

可以将多个标识字段添加到架构。 非主标识通常称为辅助标识。 要将离线购买与在线行为相关联，我们将crmId作为次要标识符添加到 `Luma Loyalty Schema` 以及后来的Web事件数据。 让我们更新 `Luma Loyalty Schema`：
...
现在，我们对身份字段进行了标记，我们可以完成Luma产品目录与事件架构之间的架构关系的设置：
...
请注意，定义关系后，将在以下两个位置中指示 **[!UICONTROL 合成]** 和 **[!UICONTROL 结构]** 模式编辑器的部分。
