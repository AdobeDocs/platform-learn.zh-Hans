...
Customer Journey Analytics使此解决方案能够基于Platform数据。 我们强烈建议观看这段时长四分钟的概述视频：
...
首先，确保选择屏幕右上角的正确数据视图。 在此示例中，要选择的数据视图为`vangeluwe - Omnichannel Data View`。
...
## 4.1.4.3计算Dimension：过滤器（分段）和日期范围

### 筛选器：计算Dimension

计算并不只适用于量度。 在开始任何分析之前，创建一些&#x200B;**计算Dimension**&#x200B;也是很有趣的。 这基本上意味着&#x200B;**区段**&#x200B;返回到Adobe Analytics。 在Customer Journey Analytics中，这些区段称为&#x200B;**筛选器**。
...
时间Dimension是另一种计算维度。 有些量度已经创建，但您还能够在数据准备阶段创建自己的自定义时间Dimension。

这些计算时间Dimension可帮助分析师和业务用户记住重要日期，并使用它们来过滤和更改报表时间。 当我们进行分析时，脑海中浮现出一些典型的问题和疑问：
...
下一步：使用Customer Journey Analytics](./ex5.md)的[4.1.5可视化图表
