---
title: 捕获Platform Web SDK的身份
description: 了解如何在XDM中捕获身份，并使用身份映射数据元素捕获用户ID。 本课程是《使用 Web SDK 实施 Adobe Experience Cloud》教程的一部分。
...
了解如何使用 Adobe Experience Platform Web SDK 捕获身份标识。捕获[Luma演示网站](https://luma.enablementadobe.com)上未经身份验证和经过身份验证的标识数据。 了解如何使用您之前创建的数据元素，通过名为身份映射的Platform Web SDK数据元素类型收集经过身份验证的数据。
...
您了解数据层是什么，熟悉[Luma演示网站](https://luma.enablementadobe.com){target="_blank"}数据层，并了解如何在标记中引用数据元素。 您必须完成本教程中之前的课程：
...
1. **[!UICONTROL 名称]**&#x200B;数据元素`Identity Map`

1. 对于&#x200B;**[!UICONTROL 扩展]**，请选择`Adobe Experience Platform Web SDK`

1. 作为&#x200B;**[!UICONTROL 数据元素类型]**，请选择`Identity map`

1. 作为&#x200B;**[!UICONTROL 命名空间]**，请选择在`lumaCrmId`配置身份[课程中创建的](configure-identities.md)命名空间。 如果下拉列表中未显示该变量，请键入该变量。

1. 作为&#x200B;**[!UICONTROL ID]**，选择在`User Id`创建数据元素[课程中创建的](create-data-elements.md#create-data-elements-to-capture-the-data-layer)数据元素。
...
>
> 此外，对于Platform应用程序的客户，Adobe建议实施[标识图形链接规则](https://experienceleague.adobe.com/zh-hans/docs/platform-learn/tutorials/identities/graph-linking-rules/overview)以防止图形折叠。
