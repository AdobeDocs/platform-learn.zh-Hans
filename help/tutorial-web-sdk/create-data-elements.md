...
了解如何在[Luma演示网站](https://luma.enablementadobe.com)上的内容、商业和身份数据的标记中创建数据元素。 然后在XDM架构中填充字段。
...
>[!IMPORTANT]
>
>本课程的数据来自`[!UICONTROL adobeDataLayer]`Luma网站[上的](https://luma.enablementadobe.com)数据层。 要查看数据层，请打开您的开发人员控制台并键入`[!UICONTROL adobeDataLayer]`，以查看完整可用的数据层。![adobeDataLayer数据层](assets/data-element-data-layer-new.png)
...
1. 将数据元素命名为 `Page Name`
1. 使用&#x200B;**[!UICONTROL JavaScript变量]** **[!UICONTROL 数据元素类型]**&#x200B;指向Luma数据层中的值： `adobeDataLayer.0.page.name`
...
* **`Ecommerce Product Name`**&#x200B;映射到`adobeDataLayer.0.ecommerce.detail.products.0.name`
* **`Ecommerce Purchase Id`**&#x200B;映射到`adobeDataLayer.0.ecommerce.purchase.actionField.id`
* **`Ecommerce Product Category`**&#x200B;使用&#x200B;**[!UICONTROL 自定义代码]** **[!UICONTROL 数据元素类型]**&#x200B;和以下自定义代码：
...
>[!CAUTION]
>
>[!UICONTROL JavaScript变量]数据元素类型将数组引用视为圆点，而不是括号，因此将用户名数据元素引用为`adobeDataLayer[0].page.name` **将不起作用**。
...
1. 命名您的数据元素`XDM Variable`。 建议您为特定于XDM的数据元素添加“XDM”前缀，以便更好地组织标记属性
...
1. 选择&#x200B;**[!UICONTROL 保存]**
   ...
   ![数据对象的变量数据元素](assets/data-element-data-variable.png)
