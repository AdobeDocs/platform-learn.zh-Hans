...
source-git-commit: 7c2f6eccd4e24f0abb6a659f4e303c425ef1563a
workflow-type: tm+mt
...
>[!NOTE]
>
>本教程使用[Luma网站](https://newluma.enablementadobe.com){target=_blank}，该网站已异步实施标记并实施了闪烁抑制。 此部分旨在了解如何通过Platform Web SDK来减少闪烁。
...
1. 选择&#x200B;**[!UICONTROL 编辑预隐藏样式]**&#x200B;按钮
   ...
   ![用于同步实施的Target预隐藏代码片段](assets/target-flicker-sync.png)
...
对于使用Luma网站的本教程，请使用在有关`lumaCrmId`标识[的课程中设置的标识符号](configure-identities.md)。
...
1. 打开`all pages - adobeDataLayer push - send event - 50`规则
...
如果您希望使用自定义代码自行呈现或操作这些决策，则可以禁用[!UICONTROL 呈现可视化个性化决策]设置。 Platform Web SDK非常灵活，并提供了这项功能来为您提供完全的控制。 您可以参阅指南以了解有关[手动渲染个性化内容](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/collection/use-cases/personalization/render-manual-propositions)的详细信息。
...
1. 导航到Adobe Target界面
1. 使用活动URL的Luma主页和Target工作区（如果有）创建体验定位(XT)活动：
...
1. 添加条件以将规则限制为Luma主页（不含查询字符串的路径等于`/`或`/index.html`）。
...
1. 将以下代码粘贴到内容框中，以更改主页中的call-to-action：
...
>[!NOTE]
>
>“已单击mbox”转化目标无法自动工作。 由于Platform Web SDK不会自动呈现自定义范围，因此它不会跟踪您选择应用内容的位置的点击次数。 您可以使用“点击”`eventType`为每个范围创建自己的点击跟踪，并使用`_experience`操作将适用的`sendEvent`详细信息包含在内。
...
有些参数对Target有用，但未从XDM对象进行映射。 这些特殊的Target参数包括：
...
1. 打开标记规则`product detail pages - adobeDataLayer push - set product details variables - 20`
1. 添加新的&#x200B;**[!UICONTROL 操作]**
...
1. 选择&#x200B;**[!UICONTROL target]**&#x200B;对象
1. 将`entity.id`设置为`Ecommerce Product Id`数据元素
1. 将`entity.name`设置为`Ecommerce Product Name`数据元素
1. 将`user.categoryId`设置为`Ecommerce Product Category`数据元素
1. 选择&#x200B;**[!UICONTROL 保留更改]**
1. 选择&#x200B;**[!UICONTROL 保存]**
...
1. 打开`all pages - adobeDataLayer push - send event - 50`规则
1. 选择`Adobe Experience Platform Web SDK - Send event`操作
1. 将`Data Variable`数据元素添加到数据字段（如果尚未添加）
...
在Luma网站上，数据层会快速加载，并在发送事件调用触发之前完成定义。 这样，我们就可以使用单次调用来获取个性化内容(例如从Adobe Target)并发送分析数据(例如发送到Adobe Analytics)。
...
第二个调用还将负责报告个性化内容的所有视图并替换任何`decisioning.propositionDisplay`调用。
...
1. 创建名为`all pages - window loaded - send event - 50`的新规则
1. 向规则添加事件。 使用&#x200B;**Core**&#x200B;扩展和&#x200B;**[!UICONTROL 已加载窗口]**&#x200B;事件类型
1. 向规则添加操作。 使用&#x200B;**Adobe Experience Platform Web SDK**&#x200B;扩展和&#x200B;**发送事件**&#x200B;操作类型
1. 选择&#x200B;**[!UICONTROL 使用引导式事件]**，然后选择&#x200B;**[!UICONTROL 收集分析]**
1. 这将锁定选定的&#x200B;**[!UICONTROL 包括渲染的建议]**&#x200B;复选框，以便发送来自决策请求的已排队显示通知。
1. 选择&#x200B;**[!UICONTROL 保留更改]**
1. **[!UICONTROL 保存]**&#x200B;规则
1. 更新并重建库
...
>[!TIP]
>
>如果您通过获取决策建议的事件之后没有Adobe Analytics事件，则它将&#x200B;**[!UICONTROL 使用引导式事件]**&#x200B;保留为未选中状态。 您需要手动选择所有选项，但它解锁了&#x200B;**[!UICONTROL 包含渲染的建议]**&#x200B;以及获取请求的选项。
...
1. 导航到[Luma演示站点](https://newluma.enablementadobe.com)
1. 导航到产品详细信息页面
