...
了解如何配置Experience PlatformWeb SDK标记扩展的隐私设置。 根据访客与同意管理平台(CMP)中的横幅的交互设置同意。

>[!NOTE]
> 
>出于演示目的，本教程使用 [克拉罗](https://heyklaro.com/) 作为CMP。 欢迎您使用Klaro或您在网站中使用的CMP来关注这些内容。
...
您应该熟悉标记以及使用Experience Platform调试器创建规则、数据元素、生成库到环境和切换标记库的步骤。

在开始配置隐私设置并创建用于设置同意的规则之前，请确保已在网站上插入同意管理平台脚本且运行正常。 CMP可以在站点开发人员的帮助下直接加载到源代码中，也可以通过标记本身加载。 这一课体现了后一种方法。
>[!NOTE]
> 
>1. 同意管理平台（或CMP）由组织用来在收集、共享或销售来自在线源（如网站和应用程序）的访客数据之前，合法地记录和管理访客的同意选择。
>
>2. 插入CMP的建议方法是直接通过源代码在标签管理器脚本之前插入。
...
1. 转到 **隐私管理器** 并根据说明创建一个实例。
1. 使用 **集成代码** 将Klaro插入到您的标记属性中（下一个练习将介绍相关说明）。
1. 跳过 **扫描** 部分，因为它将检测Luma演示网站上经过硬编码的标记属性，而不是您为本教程构建的标记属性。
1. 添加一个名为的服务 `aep web sdk` 并打开 **服务默认状态**. 启用时，默认同意值为 `true`，否则为 `false`. 当您想要确定您的Web应用程序的默认同意状态（在访客同意之前）时，此配置非常方便。 例如：
...
>[!WARNING]
>
>实施同意管理平台的最佳实践通常是加载CMP _早于_ 正在加载您的标记管理器。 为便于阅读本教程，您将加载CMP _替换为_ 标记管理器。 本课程旨在向您说明如何使用Platform Web SDK中的同意功能，但不应将其作为正确配置Klaro或任何其他CMP的指南。
...
现在，请保存此规则并将其构建到开发库中，通过将标记库从Luma网站切换到您自己的网站来验证显示同意横幅。 您应会在网站上看到如下所示的CMP横幅。 要检查当前访客的同意权限，您可以在浏览器控制台上使用以下代码片段。
...
GDPR、CCPA等隐私行为在如何设计同意实施方面起着至关重要的作用。 在本课程中，您将探索访客如何与两个最显眼隐私行为下的同意横幅进行交互。
![同意方案](assets/consent-scenarios.jpeg)


### 场景1：隐含的选择加入

隐含的选择加入意味着在收集访客的数据之前，企业不需要获得访客的同意（或“选择加入”），因此默认情况下，网站的所有访客都被视为选择加入。 但是，访客可以通过同意横幅拒绝Cookie来选择退出。 此用例类似于CCPA。

现在，您将为此方案配置并实施同意：

1. 在 **[!UICONTROL 隐私]** 部分，确保Experience PlatformWeb SDK标记扩展  **[!UICONTROL 默认同意]** 设置为 **[!UICONTROL In]** ：
   ...
   >[!NOTE]
   > 
   >对于动态解决方案，请选择“提供数据元素”选项，然后传递一个返回值的数据元素 ```klaro.getManager().consents```
   >
   >如果在源代码中插入CMP，则使用此选项 *早于* 标记嵌入代码，以便在Experience PlatformWeb SDK扩展开始加载之前提供默认同意。 在我们的示例中，我们无法使用此选项，因为CMP是随标记加载的，而不是在标记之前。



2. 保存此更改并将其生成到您的标记库
3. 在Luma演示网站上加载标记库
4. 在Luma网站上启用标记调试并重新加载页面。 在浏览器的开发人员控制台中，您应看到defaultConsent等于 **[!UICONTROL In]**
...
1. 创建一个规则，该规则会在访客单击时触发 **我拒绝**.  将此规则命名为： `all pages - click consent banner - set consent "out"`
...
通过转到Luma演示站点进行验证，拒绝Cookie，并确认在选择退出后没有触发任何Web SDK请求。

### 场景2：隐含的选择退出


隐含的选择退出意味着访客在默认情况下应被视为选择退出，而不应设置Cookie。 不应触发Web SDK请求，除非访客决定通过同意横幅接受Cookie来手动选择加入。 您可能需要在GDPR适用的欧盟地区处理此类用例。

以下是如何为隐含的选择退出方案设置配置的：

1. 在Klaro中，关闭 **服务默认状态** 在您的 `aep web sdk` 服务并保存更新的配置。

1. In **[!UICONTROL 隐私]** Experience Platform部分，将默认同意设置为 **[!UICONTROL 去话]** 或 **[!UICONTROL 待处理]** 根据需要。

   ![同意AEP扩展隐私配置](assets/consent-implied-opt-out.png)

1. **保存** 更新了标记库的配置并重建它。

   使用此配置时，Experience PlatformWeb SDK可确保不会触发任何请求，除非同意权限更改为 **[!UICONTROL In]**. 由于访客通过选择加入手动接受Cookie，可能会发生这种情况。

1. 在Debugger中，确保Luma网站已映射到您的标记属性，并且标记控制台日志记录已打开。
1. 使用浏览器的开发人员控制台 **清除站点数据** 在 **应用程序** > **存储**

1. 重新加载Luma网站，您应该会看到 `defaultConsent` 设置为 **[!UICONTROL 去话]** 并且尚未发出Web SDK请求

   ![同意默示选择退出](assets/consent-implied-out-cmp.png)

如果访客决定选择加入（接受跟踪Cookie），您必须更改同意并将其设置为 **[!UICONTROL In]**. 以下是如何使用规则执行此操作：

1. 创建一个规则，该规则会在访客单击时触发 **没关系**.  将此规则命名为： `all pages - click consent banner - set consent "in"`
...
1. 使用Experience PlatformWeb SDK添加操作 [!UICONTROL 扩展]， **[!UICONTROL 操作类型]** 之 **[!UICONTROL 设置同意]**， **[!UICONTROL 一般同意]** 作为 **[!UICONTROL In]**.

   ![同意规则选择加入操作](assets/consent-rule-optin-action.png)

   这里需要注意的一点是 [!UICONTROL 设置同意] 操作将是第一个发出并建立身份的请求。 因此，对第一个请求本身同步身份可能很重要。 可以将身份映射添加到 [!UICONTROL 设置同意] 操作。

1. 选择 **[!UICONTROL 保存到库并生成]**：

   ![同意规则选择退出](assets/consent-rule-optin-saveAndBuild.png)

1. **[!UICONTROL 保存]** 将规则复制到库中，然后重新构建该规则。
