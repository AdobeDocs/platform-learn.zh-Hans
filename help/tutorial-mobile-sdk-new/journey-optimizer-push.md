...
了解如何使用Platform Mobile SDK和Journey Optimizer为移动应用程序创建推送消息。
...
您刚刚创建了一个事件配置，该配置基于您之前在本教程中创建的“移动应用程序体验事件”架构。 此事件配置将使用您的移动应用程序标识符筛选传入体验事件，因此您确保只有从您的移动应用程序启动的事件才会触发您将在下一步中构建的历程。 在现实场景中，您可能希望从外部服务发送推送通知，但可以应用相同的概念：从外部应用程序将体验事件发送到具有特定字段的Experience Platform，您可以在这些事件触发历程之前对其应用条件。
